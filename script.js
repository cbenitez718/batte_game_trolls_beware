...
function startBattle() {
	// Clear anu old messages in the document from previous games
	...
	switch(action) {
		...
		default: {
			// Wer're here because the user has not entered a valid choice at the original prompt
...
function fightBattle() {
	...
	let skill = window.prompt("Are you a skilled fighter? (YES of NO)").toUpperCase();
	let strong = window.prompt("Do you think you are stronger than a troll? (YES or NO)").toUpperCase();

	// logic that analyzes the reponses and creates the outcome
	if(skill === "YES" || strong === "YES") {
		// the user siad YES to at least one prompt
...
function runBattle(){
	...
	if(fast === "YES") {
		...
		document.getElementById("defeat").innerHTML = "If your going to run, at least be faster than a troll.<br/>You have lost!";
		document.getElementById("lose").play();
		document.getElementById("button").textContent = "Try Again?";
	}
}
...
function bribeBattle() {
	...
	if(money === "YES") {
		// They said they had money, now asl for the amount
		...
		document.getElementById("defeat").innerHTML = "&#128128 The troll needs more than that!<br/>Your fate is sealed!";
		document.getElementById("lose").play();
		document.getElementById("button").textContent = "Try Again?";
	} // end of nested else
	} else {
